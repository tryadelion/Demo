...
export class OffersService {
  ...
  private OFFERS_URL = 'list/offers';
  private UPDATE_OFFER_URL = 'update/offers';
  ...
  findAll(token,uid) {
    ...
    var url = this.constantsService.getApiUrlBase() + this.OFFERS_URL;
  ...
  addToFavourites(id,token,uid){
    ...
    let options = new RequestOptions({ headers: headers });
    console.log(body);
    var url = this.constantsService.getApiUrlBase() + this.UPDATE_OFFER_URL;
    return this.http.post(url, body, options)
      .map(res => res.json())
      .catch(this.handleError);
  }
  ...
  contactOffer(id,name,email,phone,message,uid,token) {
    let body = new URLSearchParams();
    body.append('uid',uid);
    body.append('id',id);
    body.append('session_token',token);
